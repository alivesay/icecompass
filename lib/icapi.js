        ...
        ws.on('message', function(data, flags) {
            ...
            if (data.id) {
                cache.get(data.id, function (err, requestCallback) {
                    /* TODO: limited support for stream_id for now.. using count: 1 */
                    if (data.result && data.result.stream_id) {
                        cache.set(data.result.stream_id, requestCallback);
                        return;
                    }
...
var apiMethods = {
    ...
    stream: {
        follow: function (options, callback) {
            rpcRequest(options.token, 'stream.follow', {
                username: options.username,
                type: 'location',
                count: 1,
                order: 'newest',
